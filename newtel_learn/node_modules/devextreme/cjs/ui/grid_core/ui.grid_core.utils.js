...
var _default = {
    ...
    getWrappedLookupDataSource: function(column, dataSource, filter) {
        ...
        var group = hasLookupOptimization ? [column.dataField, column.displayField] : column.dataField;
            ...
            load: function(loadOptions) {
                ...
                dataSource.load({
                    ...
                    if (hasLookupOptimization) {
                        ...
                        var newDataSource = new _data_source.DataSource(_extends({}, loadOptions, {
                            store: new _array_store.default({
                                data: lookupItems,
                                key: column.lookup.valueExpr
                            })
                        }));
                        newDataSource.load(loadOptions).done(d.resolve).fail(d.fail)
                    } else {
                        var _filter = _this.combineFilters(items.map((function(data) {
                            return [column.lookup.valueExpr, data.key]
                        })), "or");
                        new _data_source.DataSource(_extends({}, lookupDataSourceOptions, loadOptions, {
                            filter: _this.combineFilters([_filter, loadOptions.filter], "and")
                        })).load(loadOptions).done(d.resolve).fail(d.fail)
                    }
                })).fail(d.fail);
                return d
            },
